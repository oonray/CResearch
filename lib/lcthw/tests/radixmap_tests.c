...
char *test_operations(){
   ...
   while(map->end > 0){
        ...
        mu_assert(check_order(map),"RadixMap did not stay sorted after delete.");
    }
...
char *all_tests(){
    test_info("INFO: Running all tests.");
    mu_suite_start();
    srand(time(NULL)); 
