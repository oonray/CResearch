...
MunitResult test_get_set(const MunitParameter params[], void* user_data_or_fixture){
		...
		char *data = Map_Get(dt->map,dt->data1);
		check(strcmp(data,dt->expect1)==0,"Got Wrong Data");

		data = Map_Get(dt->map,dt->data2);
		check(strcmp(data,dt->expect2)==0,"Got Wrong Data");

		data = Map_Get(dt->map,dt->data3);
		check(strcmp(data,dt->expect3)==0,"Got Wrong Data");
