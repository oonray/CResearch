...
inline Vector *Vector_New(size_t element_size,size_t initial_max){
...
inline void Vector_Destroy(Vector *arr){
		if(arr){
			if(arr->contents){
				free(arr->contents);
				free(arr);
...
inline void Vector_Clear(Vector *arr){
	int i = 0;
...
inline int Vector_Resize(Vector *arr,size_t new_size){
		arr->max = new_size;
...
inline int Vector_Expand(Vector *arr){
...
inline void *Vector_Remove(Vector *arr,int i){
	void *el = arr->contents[i]; 
	arr->contents[i] = NULL;
	return el;
}

inline void *Vector_Pop(Vector *arr){
...
inline void Vector_Clear_Destroy(Vector *arr){
		Vector_Clear(arr);
		Vector_Destroy(arr);
}

inline void Vector_Set(Vector *arr,int i, void *el){
...
inline void *Vector_Get(Vector *arr,int i){
...
inline void *Vector_Create(Vector *arr){
