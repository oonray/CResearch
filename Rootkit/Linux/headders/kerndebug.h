...
#define debug(M, ...)
#else 
#define debug(M, ...) printk(KERN_INFO "%s[.]%s DEBUG %s:%d:%s: " M "\n",KCYN,KNRM __FILE__, __LINE__, __func__, ##__VA_ARGS__) 
#endif

#define log_err(M, ...) printk(KERN_ALERT "%s[-]%s ERROR (%s:%d:%s) " M "\n",KRED,KNRM,__FILE__, __LINE__, __func__, ##__VA_ARGS__) 

#define log_warn(M, ...) printk(KERN_ALERT "%s[!]%s WARNING (%s:%d:%s) " M "\n",KMAG,KNRM, __FILE__, __LINE__, __func__, ##__VA_ARGS__) 
